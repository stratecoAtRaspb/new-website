<script lang="ts">
  import BasicContentArea from '$lib/components/basic-content-area.svelte';
  import IntroBox from '$lib/components/intro-box.svelte';
  import Section from '$lib/components/section.svelte';
  import Stage from '$lib/components/stage.svelte';
  import { m } from '$lib/paraglide/messages';
  import { localizeHref } from '$lib/paraglide/runtime';
  import { resolve } from '$app/paths';
</script>

<svelte:head>
  <title>{m.topics_financeAndControlling_dataManagement_title()}</title>
  <meta name="description" content={m.topics_financeAndControlling_dataManagement_title()} />
</svelte:head>

<Stage>
  <IntroBox title={m.navigation_topics_dataManagement()} description={m.aboutUs_introDescription()}></IntroBox>
</Stage>

{#snippet left()}
  <div class="prose">
    <h2>{m.navigation_topics_dataManagement()}</h2>
    <h3>Kompetenz</h3>
    <p>
      STRATECO Data Management Lösungen helfen dem Kunden sichere Entscheidungen aufgrund von zuverlässigen Informationen zu treffen, um sich im internationalen
      Markt wettbewerbsfähig und<br /> erfolgreich zu positionieren. STRATECO realisiert auf der Basis von Microsoft Technologie geeignete<br /> integrierte Finanz-
      und Data Management Lösungen, die dem Kunden schnell verlässliche und maß-geschneiderte Informationen zur Verfügung stellen.
    </p>
    <h3>Schwerpunkt</h3>
    <ul>
      <li>Business-Intelligence</li>
      <li>Database Management</li>
    </ul>
    <h3>Lösungen</h3>
    <ul>
      <li>Data Warehousing</li>
      <li>Business-Intelligence</li>
      <li>OLAP</li>
      <li>Data Mining</li>
      <li>Reporting</li>
    </ul>
    <h3>Branchen</h3>
    <ul>
      <li><a title="Financial Institutions " href="/de/kompetenzen/financial-institution">Financial Institutions</a></li>
      <li><a title="Healthcare" href="/de/kompetenzen/pharma-healthcare">Healthcare</a></li>
      <li><a title="Telecoms, Media, High Tech" href="/de/kompetenzen/telecoms-media-hightech">Telecoms, Media, High Tech</a></li>
      <li><a title="Travel, Transport &amp; Logistics" href="/de/kompetenzen/travel-transport-logistics">Travel, Transport &amp; Logistics</a><br /></li>
      <li><a title="Sonstige" href="/de/kompetenzen/other">Sonstige</a></li>
    </ul>
  </div>

  <!-- eslint-disable-next-line @typescript-eslint/no-explicit-any -->
  <a href={ resolve(localizeHref('/topics/finance-controlling') as any) }>Zurück zu Übersicht</a>
{/snippet}
{#snippet right()}{/snippet}
<Section>
  <BasicContentArea {left} {right} />
</Section>
